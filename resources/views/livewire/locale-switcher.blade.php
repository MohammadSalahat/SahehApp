<div class="relative inline-block text-left" x-data="{ open: false }">
    <div>
        <button @click="open = !open" type="button"
            ...
            <svg class="w-5 h-5 {{ $currentLocale === 'ar' ? 'ml-2' : 'mr-2' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Dropdown arrow -->
            <svg class="{{ $currentLocale === 'ar' ? 'mr-2' : 'ml-2' }} -mr-1 h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
    ...
    <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-100"
        ...
        <div class="py-1" role="none">
            @foreach ($availableLocales as $locale => $localeData)
                <button wire:click="switchLocale('{{ $locale }}')" @click="open = false"
                    ...
                    <!-- Locale flag/icon -->
                    <svg class="w-5 h-5 {{ $currentLocale === 'ar' ? 'ml-3' : 'mr-3' }} text-gray-400 group-hover:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129" />
                    </svg>
                    ...
                    @if ($locale === $currentLocale)
                        <svg class="w-5 h-5 {{ $currentLocale === 'ar' ? 'mr-auto' : 'ml-auto' }} text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
